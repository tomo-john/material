...
ssh-agentを起動するとバックグラウンドで動作し、`ssh-add`や`ssh`コマンドはssh-agentが作成したUNIXドメインソケット(プロセス間で通信するためのインターフェース)を介してssh-agentと通信を行う。

=> このソケットまでのパスは環境変数`SSH_AUTH_SOCK`に格納される
